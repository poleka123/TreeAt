...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(self, c_in, c_out, dia=1):
        ...
        self.c_in = c_in
        self.conv = nn.Conv2d(
            c_in, c_out, (1, 2), 1, dilation=dia, padding=(0, 0)
        )

    def forward(self, x):
        return torch.relu(self.conv(x.permute(0, 3, 1, 2))).permute(0, 2, 3, 1)
...
class SpatioConvLayer(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(0, 3, 2, 1) # change shape from(b, n, t, c) to (b, c, t,n)
        x = x.transpose(0, 3)
        x = x.transpose(1, 3)
        output = self.gc(self.g, x)
        output = output.transpose(1, 3)
        output = output.transpose(0, 3)
        return torch.relu(output.permute(0, 3, 2, 1))
...
class OutputLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fc(x_t2.reshape(b, n, c))


class STGCN_WAVE(nn.Module):
    def __init__(
        self, c, T, n, Lk, control_str="TNTSTNTST"
    ):
