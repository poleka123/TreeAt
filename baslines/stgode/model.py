...
from ..stgode.odegcn import ODEG
...
class ODEGCN(nn.Module):
    ...
    def __init__(self, num_nodes, num_features, num_timesteps_input,
        ...
        self.sp_blocks = nn.ModuleList(
            [nn.Sequential(
                STGCNBlock(in_channels=num_features, out_channels=[32, 32, 32],
                num_nodes=num_nodes, A_hat=A_sp_hat),
                STGCNBlock(in_channels=32, out_channels=[32, 32, 32],
                num_nodes=num_nodes, A_hat=A_sp_hat)) for _ in range(1)
            ])
        # semantic graph
        # self.se_blocks = nn.ModuleList([nn.Sequential(
        #         STGCNBlock(in_channels=num_features, out_channels=[32, 16, 32],
        #         num_nodes=num_nodes, A_hat=A_se_hat),
        #         STGCNBlock(in_channels=32, out_channels=[32, 16, 32],
        #         num_nodes=num_nodes, A_hat=A_se_hat)) for _ in range(1)
        #     ])

        self.pred = nn.Sequential(
            # nn.Linear(num_timesteps_input * 16, num_timesteps_output * 16),
            # nn.ReLU(),
